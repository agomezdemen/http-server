...
// Shared HTTP value types used by request, response, and parser code.
  ...
  enum class Version : std::uint8_t { http_1_1, unknown };
  ...
  // Convert internal enum values to wire-format text.
  ...
  // Parse wire-format tokens into internal enum values.
  ...
  // Return the numeric status code used in an HTTP status line.
  constexpr auto status_code(Status status) noexcept -> int { return static_cast<int>(status); }
}
#endif
