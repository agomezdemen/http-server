...
TcpListener::TcpListener(Endpoint ep, int backlog)
  ...
  if (::setsockopt(sfd_.get(), SOL_SOCKET, SO_REUSEADDR, &opt,
  ...
  const int ip_result{::inet_pton(AF_INET, ep_.host().c_str(), &addr.sin_addr)};
  ...
  if (::bind(sfd_.get(), reinterpret_cast<const sockaddr*>(&addr),
...
auto TcpListener::endpoint() const noexcept -> const Endpoint& { return ep_; }
